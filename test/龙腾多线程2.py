...
class WorkerThread(QThread):
    ...
    def 监控线程(self):
        ...
        while True:
            ...
            s = time.perf_counter()
            ret = self.大漠对象.FindStrEx(*config.大范围识别玩家区域, "D4|D5|D6|Z4|Z5|Z6|F4|F5|F6", 'ffffff-000000', 1)
            print(ret)
            if ret != '':
                ret1 = self.大漠对象.ExcludePos(ret, 0, *config.排除人物等级区域)
                if ret1 != '':
                    frame,players = self.更新地图_玩家点()
                    人物x,人物y = self.识别人物当前坐标(*config.识别人物坐标区域)
    ...
    def 识别血量(self):
        识别结果 = self.大漠对象.Ocr(*config.识别人物血量区域, "#255-50|#253-50", 1)
        if 识别结果 != '':
            识别结果 = 识别结果.strip(":")
            分割结果 = 识别结果.split('/')
            当前血量 = int(分割结果[0])
    ...
    def 更新地图_玩家点(self):
        frame = self.map_img.copy()
        玩家坐标列表 = self.识别玩家坐标(*config.大范围识别玩家区域)
        if 玩家坐标列表:
            print("玩家坐标列表", 玩家坐标列表)
    ...
    def 识别人物当前坐标(self,x1, y1, x2, y2):
        self.大漠对象.UseDict(0)
        识别结果 = self.大漠对象.Ocr(x1, y1, x2, y2, "#255-50|#253-50", 1)         # 正版龙腾范围 48, 1057, 108, 1078
    ...
    def 识别玩家坐标(self,x1,y1,x2,y2):
        try:
            ...
            if ret != "" :
                ret1 = self.大漠对象.ExcludePos(ret, 0, *config.排除人物等级区域)
                if ret1 != '':
                    人物坐标x, 人物坐标y = self.识别人物当前坐标(*config.识别人物坐标区域)
                    玩家坐标列表 = []
                    ret1_list = ret1.split('|')
                    for i in ret1_list:
                        i_list = i.split(',')
                        玩家屏幕x = int(i_list[1]) + config.等级与中心点的X偏移
                        玩家屏幕y = int(i_list[2]) + config.等级与中心点的Y偏移
    ...
    def 押镖(self):
        try:
            for i in range(30):
                # 走到可以接镖车的位置
                while True:
                    识别结果 = self.大漠对象.Ocr(*config.识别人物坐标区域, "#255-50|#253-50", 1)
                    if 识别结果 != '':
                        分割结果 = 识别结果.split(':')
                        人物x = int(分割结果[0])
                        人物y = int(分割结果[1])
    ...
    def 沿路径控制人物行走_打怪线程(self, path,
        ...
        依赖外部接口：
            - 识别人物当前坐标(48, 1057, 108, 1078) -> (x, y) 或 0
        ...
        # ===== 入场检查：人物是否在路径起点附近 =====
        cur = self.识别人物当前坐标(*config.识别人物坐标区域)  # 这里用你统一后的参数
        ...
        try:
            while True:
                ...
                cur = self.识别人物当前坐标(*config.识别人物坐标区域)
                # OCR 失败直接下一轮
                if not cur or cur[0] < 0 or cur[1] < 0:
                    time.sleep(0.05)
                    continue
...
class MyWindow(QMainWindow):
    ...
    def 识别人物当前坐标(self,x1, y1, x2, y2):
        dms_a[0].UseDict(0)
        识别结果 = dms_a[0].Ocr(x1, y1, x2, y2, "#255-50|#253-50", 1)         # 正版龙腾范围 48, 1057, 108, 1078
        # 识别结果 = self.大漠对象.Ocr(84,1061,128,1077, "#255-50|#253-50", 1)     # 单机版龙腾范围 84,1061,128,1077
        if 识别结果 != '':
            # print('识别结果',识别结果)
            识别结果 = 识别结果.strip(":")
    ...
    def 识别玩家坐标(self,x1,y1,x2,y2):
        try:
            ...
            # 7 83
            if ret != "" :
                ret1 = dms_a[0].ExcludePos(ret, 0, *config.排除人物等级区域)
                print(ret1)
                if ret1 != '':
                    人物坐标x, 人物坐标y = self.识别人物当前坐标(*config.识别人物坐标区域)
                    玩家坐标列表 = []
